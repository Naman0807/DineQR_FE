...
export function AdminLayout() {
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      {/* Sidebar */}
      <aside className={`
        fixed top-0 left-0 z-40 h-full bg-white shadow-lg transform transition-all duration-200
        ${sidebarCollapsed ? 'w-20' : 'w-64'}
        lg:translate-x-0
        ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}
      `}>
        <div className="p-4 sm:p-6 border-b flex items-center justify-between">
          <div className={`transition-all duration-200 ${sidebarCollapsed ? 'hidden lg:block' : ''}`}>
            <h1 className={`font-bold text-orange-500 ${sidebarCollapsed ? 'text-base' : 'text-xl sm:text-2xl'}`}>
            ...
            {navItems.map(item => (
              <li key={item.to}>
                ...
                >
                  ...
                  <span className={`transition-all duration-200 ${sidebarCollapsed ? 'hidden lg:hidden' : ''}`}>
                    {item.label}
                  </span>
                </NavLink>
              </li>
      ...
      {/* Main Content */}
      <main className={`min-h-screen transition-all duration-200 ${sidebarCollapsed ? 'lg:ml-20' : 'lg:ml-64'}`}>
        <div className="p-4 sm:p-5 lg:p-6 pt-20 lg:pt-6 pb-24 lg:pb-6">
